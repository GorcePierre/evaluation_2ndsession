...
/**
 * on pourrait appeler cette classe GestionJDBC son r�le consiste � g�rer la
...
public class AccesBD {
	 ...
	 * connexion entre notre programme java et la base de donn�es.
	 */
	...
	/**
	 * m�thode d'ex�cution d'une requ�te (SELECT) pour renvoyer un objet de type
	 * ResultSet
	 * 
	...
	{
		/*
		 * On d�clare un objet de type Statement que l'on nomme instruction. Cet objet
		 * soumet la requ�te � la base de donn�es dans MySQL. On d�clare un objet de
		 * type ResultSet que l'on nomme resultat. cet objet est retourn� pour
		 * encapsuler les r�sultats de la requ�te. Il va nous permettre de manipuler les
		...
		try {
			 ...
			 *
			 *
			 * TYPE_SCROLL_INSENSITIVE : Cette valeur indique que le curseur peut �tre
			 * d�plac� dans les deux sens, mais aussi arbitrairement (de mani�re absolue ou
			 * relative). Le terme insensitive indique que le ResultSet est insensible aux
			 * modifications des valeurs dans la base de donn�es. Cela d�finit en fait une
			 * vue statique des donn�es contenues dans le ResultSet.
			 *
			 * CONCUR_UPDATABLE : Cette valeur indique que l'on peut modifier les donn�es de
			...
			/*
			 * On peut traduire Statement par ordre ou instruction. La m�thode
			 * createStatement() nous retourne un objet de type Statement. Nous l'avons
			 * appel� avec la m�thode getConnection() qui nous renvoie un objet de type
			 * Connexion. D�s lors, nous pouvons utiliser l'objet instruction pour
			 * interroger la base de donn�es Oracle.
			 *
			 */
			 ...
			 * comme param�tre, la requete que nous voulons ex�cuter. L'objet resultat
			 * contient le r�sultat de l'ex�cution de la requ�te.
			 */
	...
	/**
	 * M�thode d'ex�cution d'une requete Update (UPDATE, INSERT, DELETE). Elle ne
	 * renvoie rien
	 * 
	 * @param requete String
	 */
